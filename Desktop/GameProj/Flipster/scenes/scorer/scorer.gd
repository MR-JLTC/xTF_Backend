...
var _time_limit: int = 0
var _time_remaining: int = 0

func update_time_remaining(time: int) -> void:
...
var _is_pair_matched: bool = false
...
func clear_new_game(target_pairs: int, time_limit: int) -> void:
	_selections.clear()
	_pairs_made = 0
	_moves_made = 0
	_target_pairs = target_pairs
	_time_limit = time_limit
	_is_pair_matched = false
...
func increment_matched_pairs(win_status: bool) -> void:
	...
	if _pairs_made >= _target_pairs:
		var time_elapsed = _time_limit - _time_remaining
		SignalManager.on_game_over.emit(_moves_made, win_status, time_elapsed)

func kill_tiles() -> void:
	for s in _selections:
		s.kill_on_success()
	_is_pair_matched = true
...
func update_selections() -> void:
	reveal_timer.start()
	_is_pair_matched = selections_are_pair()
	if _is_pair_matched == true:
		kill_tiles()
...
func _on_reveal_timer_timeout():
	if _is_pair_matched == false:
		hide_selections()
		SoundManager.play_sound(SoundManager.SOUND_NO_MATCH)
	_selections.clear()
