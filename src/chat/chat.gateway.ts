...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    ...
    try {
      ...
      conversations.forEach((conv) => {
        ...
        console.log(
          `Socket - User ${userId} auto-joined room ${conv.conversation_id}`,
        );
      });
      ...
      const deliveredMessages = await this.chatService.markMessagesAsDelivered(
        Number(userId),
      );
      if (deliveredMessages.length > 0) {
        console.log(
          `Socket - Marked ${deliveredMessages.length} messages as delivered for user ${userId}`,
        );
  ...
  handlePingOnlineUsers(
    ...
    console.log(
      `ChatGateway - User ${client.data.user?.sub} requested online status for ${data.userIds.length} users. ${online.length} are online.`,
    );
    return online;
  }
  ...
  async handleMessage(
    ...
    console.log(
      `ChatGateway - Received sendMessage from ${senderId} for conv ${roomName}`,
    );
    ...
    try {
      ...
      const partnerId =
        Number(conversation.tutor_id) === Number(senderId)
      ...
      const isPartnerOnline =
        this.server.sockets.adapter.rooms.get(`user_${partnerId}`)?.size > 0;
      ...
      console.log(
        `ChatGateway - Message saved: ${message.message_id} (${initialStatus}). Broadcasting...`,
      );
  ...
  async handleMarkSeen(
    ...
    console.log(
      `ChatGateway - Marking messages seen for conv ${conversationId} by user ${userId}`,
    );

    const seenMessages = await this.chatService.markMessagesAsSeen(
      conversationId,
      userId,
    );
  ...
  handleTyping(
    ...
    client
      ...
      .emit("typing", { userId: senderId, isTyping: data.isTyping });
  }
}
