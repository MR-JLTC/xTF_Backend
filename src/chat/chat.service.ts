...
export class ChatService {
  ...
  async createConversation(user1Id: number, user2Id: number) {
    ...
    if (role1 === role2 && role1 !== "admin") {
      // Admins might bridge, but req says Strict Rules. Let's assume strict.
      // Actually strictly follow: Tutor <-> Tutee only.
      throw new ForbiddenException(
        "Chats are only allowed between a Tutor and a Tutee.",
      );
    }

    // Specifically block Tutor-Tutor and Tutee-Tutee
    if (
      (role1 === "tutor" && role2 === "tutor") ||
      (role1 === "tutee" && role2 === "tutee")
    ) {
      throw new ForbiddenException(
        "Chats are only allowed between a Tutor and a Tutee.",
      );
  ...
  async getConversations(userId: number) {
    ...
    return this.conversationRepository.find({
      where: [{ tutor_id: userId }, { tutee_id: userId }],
      relations: ["tutor", "tutee"],
  ...
  async sendMessage(
    ...
    if (
      conversation.tutor_id !== senderId &&
      conversation.tutee_id !== senderId
    ) {
