    ...
    try {
      ...
      if (results.port5432 === "OPEN" && results.port6543 === "CLOSED") {
        results.recommendation =
          "Port 6543 is blocked. Change DATABASE_URL to use port 5432.";
      } else if (results.port6543 === "OPEN") {
        results.recommendation =
          "Port 6543 is open. Connection should work if SSL is configured correctly.";
      } else {
        results.recommendation =
          "Both ports blocked. Check if Supabase project is PAUSED or Network Restrictions are active.";
      }
