...
export class EmailVerificationService {
  ...
  async sendVerificationCode(
    ...
    if (!user_type || !["tutor", "tutee", "admin"].includes(user_type)) {
      ...
      throw new BadRequestException(
        "User type is required and must be tutor, tutee, or admin",
      );
    }
    ...
    let verificationEntry =
      await this.emailVerificationRegistryRepository.findOne({
  ...
  async getEmailVerificationStatus(
    ...
    if (!verificationEntry) {
      console.log(
        "❌ No verification entry found for email/user_type:",
        trimmedEmail,
        user_type,
      );
    ...
    if (
      !verificationEntry.is_verified &&
      verificationEntry.verification_code &&
      verificationEntry.verification_expires
    ) {
      const now = new Date();
  ...
  async verifyEmailCode(
    ...
    if (!entry) {
      ...
      throw new NotFoundException(
        "No verification request found for this email and user type.",
      );
    }

    console.log("✅ Verification entry found:", {
  ...
  private async sendVerificationEmail(
    ...
    try {
      ...
      if (!sent) {
        throw new Error(
          "Email service failed to send email. Check backend logs for Gmail API errors.",
        );
      }
