...
export class UsersController {
  ...
  async findAll() {
    ...
    return users.map((u) => ({
      ...
      university_id:
        u.student_profile?.university_id ||
        u.tutor_profile?.university_id ||
        null,
      course_id:
        u.student_profile?.course_id || u.tutor_profile?.course_id || null,
      university_name:
        u.student_profile?.university?.name ||
        u.tutor_profile?.university?.name ||
        "N/A",
      university:
        u.student_profile?.university || u.tutor_profile?.university || null,
  ...
  async updateAdminGcashNumber(
    @Param("id") id: string,
    @Body() body: { gcash_number: string },
  ) {
  ...
  async submitBookingFeedback(
    ...
    const userId = req.user?.user_id;
    return this.usersService.submitBookingFeedback(
      +bookingId,
      userId,
      body.rating,
      body.comment || "",
  ...
  async confirmBookingCompletion(
    @Param("bookingId") bookingId: string,
    @Req() req: any,
  ) {
    const userId = req.user?.user_id;
