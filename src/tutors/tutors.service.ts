...
export class TutorsService {
  ...
  async updateStatus(
    ...
    if (status === "rejected") {
      ...
      if (
        ...
        console.log(
          `[TutorService] REJECTION - Admin notes received: "${adminNotes}"`,
        );
        console.log(
          `[TutorService] REJECTION - Trimmed admin notes to save: "${adminNotesToSave}"`,
        );
        console.log(
          `[TutorService] REJECTION - Setting tutor.admin_notes property = "${adminNotesToSave}"`,
        );
        console.log(
          `[TutorService] REJECTION - This MUST be saved to tutors.admin_notes column in database`,
        );
      } else {
        console.error(
          `[TutorService] ERROR: Rejecting tutor ${id} but adminNotes is missing or empty!`,
        );
        console.error(`[TutorService] Received adminNotes:`, adminNotes);
        console.error(
          `[TutorService] This should not happen - modal should require rejection reason.`,
        );
      ...
      tutor.admin_notes = adminNotesToSave;
      console.log(
        `[TutorService] Admin notes provided for ${status}: "${adminNotesToSave || "null"}"`,
      );
    }

    // CRITICAL: Save tutor entity to database FIRST - this MUST persist tutor.admin_notes to tutors.admin_notes column
    console.log(
      `[TutorService] BEFORE SAVE - tutor.admin_notes = "${tutor.admin_notes || "null"}"`,
    );
    let savedTutor = await this.tutorsRepository.save(tutor);
    console.log(
      `[TutorService] AFTER SAVE - savedTutor.admin_notes = "${savedTutor.admin_notes || "null"}"`,
    );
    ...
    if (status === "rejected") {
      if (adminNotesToSave !== null) {
        ...
        console.log(
          `[TutorService] Explicitly updated tutors.admin_notes column using update() method`,
        );
        console.log(
          `[TutorService] Update result affected rows:`,
        ...
        if (reloadedTutor) {
          console.log(
            `[TutorService] VERIFICATION - Reloaded tutor ${id} from database after explicit update:`,
          );
          console.log(`[TutorService]   - Status: ${reloadedTutor.status}`);
          console.log(
            `[TutorService]   - Admin Notes from tutors.admin_notes column:`,
            reloadedTutor.admin_notes
              ? `"${reloadedTutor.admin_notes}"`
              : "NULL",
          );
          ...
          if (
            !reloadedTutor.admin_notes ||
            reloadedTutor.admin_notes !== adminNotesToSave
          ) {
            console.error(
              `[TutorService] ERROR: Admin notes NOT saved correctly!`,
            );
            console.error(`[TutorService] Expected: "${adminNotesToSave}"`);
            console.error(
              `[TutorService] Actual from DB: "${reloadedTutor.admin_notes || "NULL"}"`,
            );
          } else {
            console.log(
              `[TutorService] SUCCESS: Admin notes verified in database!`,
            );
          }
        ...
        console.error(
          `[TutorService] ERROR: Cannot update admin_notes - adminNotesToSave is null for rejected status!`,
        );
      }
    }
    ...
    console.log(
      `[TutorService]   - Admin Notes (tutors.admin_notes column):`,
    ...
    if (status === "approved") {
      ...
      try {
        ...
        console.error(
          "Failed to send tutor application approval email:",
          error,
        );
      ...
      const notesForEmail =
        savedTutor.admin_notes && savedTutor.admin_notes.trim().length > 0
          ? savedTutor.admin_notes.trim()
          : undefined;

      console.log(
        `[TutorService] Sending rejection email to ${(tutor.user as any)?.email}`,
      );
      console.log(
        `[TutorService] Admin notes from database (admin_notes column):`,
        notesForEmail
          ? `"${notesForEmail.substring(0, 50)}${notesForEmail.length > 50 ? "..." : ""}"`
          : "none",
      );
      ...
      try {
        ...
        console.log(
          `[TutorService] Rejection email sent successfully to ${(tutor.user as any)?.email}`,
        );
      } catch (error) {
        console.error(
          "[TutorService] Failed to send tutor application rejection email:",
          error,
        );
  ...
  async getTutorByEmail(
    email: string,
  ): Promise<{ tutor_id: number; user_id: number; user_type: string }> {
  ...
  async updateExistingUserToTutor(
  ...
  ): Promise<{ success: true; tutor_id: number }> {
    const user = await this.usersRepository.findOne({
    ...
    if (
      !resolvedCourseId &&
      data.course_name &&
      data.course_name.trim().length > 0
    ) {
      const uni =
        tutor.university ||
        (await this.universitiesRepository.findOne({
          where: { university_id: tutor.university_id },
        }));
      if (uni) {
        const existingCourse = await this.coursesRepository.findOne({
          where: {
            course_name: data.course_name.trim(),
            university: { university_id: uni.university_id },
          },
  ...
  async applyTutor(data: {
    ...
    gcash_number?: string;
    session_rate_per_hour?: number;
  }): Promise<{ success: true; user_id: number; tutor_id: number }> {
    ...
    if (data.university_id) {
      universityEntity = await this.universitiesRepository.findOne({
        where: { university_id: data.university_id },
      });
      if (!universityEntity) {
    ...
    if (
      ...
      const existingCourse = await this.coursesRepository.findOne({
        where: {
          course_name: data.course_name.trim(),
          university: { university_id: universityEntity.university_id },
        },
        relations: ["university"],
      });
      if (existingCourse) {
    ...
    if (resolvedCourseId) {
      courseEntity = await this.coursesRepository.findOne({
        where: { course_id: resolvedCourseId },
      });
    }
    ...
    const tutor = this.tutorsRepository.create({
      ...
      ...(universityEntity && {
        university: universityEntity,
        university_id: universityEntity.university_id,
      }),
      ...(courseEntity && {
        course: courseEntity,
    ...
    return {
      ...
      tutor_id: (savedTutor as any).tutor_id,
    };
  }
  ...
  async saveDocuments(tutorId: number, files: any[]) {
    console.log(
      `[TutorsService] saveDocuments called for tutorId: ${tutorId} with ${files?.length || 0} files`,
    );
    const tutor = await this.tutorsRepository.findOne({
      where: { tutor_id: tutorId },
    });
  ...
  async saveProfileImage(tutorId: number, file: any) {
    console.log(
      `[TutorsService] saveProfileImage called for tutorId: ${tutorId}, file provided: ${!!file}`,
    );
    ...
    if (!file) {
      ...
      await this.usersRepository.update(
        { user_id: userId },
        { profile_image_url: placeholderUrl },
      );
  ...
  async saveGcashQR(tutorId: number, file: any) {
    console.log(
      `[TutorsService] saveGcashQR called for tutorId: ${tutorId}, file provided: ${!!file}`,
    );
    // First try to find tutor by tutor_id, if not found, try by user_id
    let tutor = await this.tutorsRepository.findOne({
      where: { tutor_id: tutorId },
    ...
    if (!file) {
      ...
      await this.tutorsRepository.update(
        { tutor_id: tutor.tutor_id },
        { gcash_qr_url: placeholderUrl },
      );
  ...
  async saveAvailability(
    tutorIdOrUserId: number,
    slots: { day_of_week: string; start_time: string; end_time: string }[],
  ) {
    console.log(
      `[TutorsService] saveAvailability called for ID: ${tutorIdOrUserId} with ${slots?.length || 0} slots`,
    );
    // Accept either tutor_id or user_id for flexibility (dashboard passes user_id)
    let tutor = await this.tutorsRepository.findOne({
      where: { tutor_id: tutorIdOrUserId },
      relations: ["user"],
    });
    if (!tutor) {
      tutor = await this.tutorsRepository.findOne({
        where: { user: { user_id: tutorIdOrUserId } },
        relations: ["user"],
      });
    }
    if (!tutor) throw new Error("Tutor not found");
    ...
    await this.availabilityRepository.delete({
      tutor: { tutor_id: tutor.tutor_id } as any,
    });
  ...
  async saveSubjects(
    ...
    if (
      providedCourseId &&
      tutorCourseId &&
      providedCourseId !== tutorCourseId
    ) {
      throw new Error(
        `Course ID mismatch: Provided course_id (${providedCourseId}) does not match tutor's course_id (${tutorCourseId}).`,
      );
    }
    ...
    if (effectiveCourseId) {
      courseEntity = await this.coursesRepository.findOne({
        where: { course_id: effectiveCourseId as any },
      });
      if (!courseEntity) {
    ...
    for (const rawName of subjectNames) {
      ...
      if (courseEntity) {
        ...
        if (!subject) {
          ...
          const existingWithDifferentCourse =
          ...
          if (existingWithDifferentCourse) {
            const existingCourseId = (existingWithDifferentCourse as any).course
              ?.course_id;
            ...
            if (
              existingCourseId &&
              existingCourseId !== courseEntity.course_id
            ) {
              console.log(
                `Subject "${name}" exists in course_id ${existingCourseId}, creating new subject for course_id ${courseEntity.course_id}`,
              );
              ...
              console.log(
                `Created new subject "${name}" with course_id ${subject?.course?.course_id || courseEntity.course_id}, subject_id: ${subject.subject_id} (duplicate name allowed for different course)`,
              );
              ...
              console.log(
                `Updated existing subject "${name}" to have course_id ${subject?.course?.course_id || courseEntity.course_id}, subject_id: ${subject.subject_id}`,
              );
            }
          } else {
            ...
            console.log(
              `Created new subject "${name}" with course_id ${subject?.course?.course_id || courseEntity.course_id}, subject_id: ${subject.subject_id}`,
            );
          }
        }
      } else {
        ...
        if (!subject) {
          ...
          subject = await this.subjectRepository.save(created);
          console.log(
            `Created new subject "${name}" without course, subject_id: ${subject.subject_id}`,
          );
        } else {
          ...
          console.log(
            `Found existing subject "${name}" without course, subject_id: ${subject.subject_id}`,
          );
        }
      }
      ...
      if (effectiveCourseId && subject) {
        ...
        if (subjectCourseId && subjectCourseId !== effectiveCourseId) {
          throw new Error(
            `Subject "${name}" belongs to course ID ${subjectCourseId}, but tutor is registered with course ID ${effectiveCourseId}. Cannot associate subject from different course.`,
          );
        }
      }
      ...
      toCreate.push(link);
      console.log(
        `Created TutorSubject link for tutor_id ${tutorId}, subject_id ${subject.subject_id}, subject_name "${name}"`,
      );
    }

    // Delete existing tutor subjects for this tutor
    const deleteResult = await this.tutorSubjectRepository.delete({
      tutor: { tutor_id: tutorId } as any,
    });
    console.log(
      `Deleted ${deleteResult.affected || 0} existing TutorSubject entries for tutor_id ${tutorId}`,
    );

    // Save new tutor subjects
    const savedTutorSubjects = await this.tutorSubjectRepository.save(toCreate);
    console.log(
      `Saved ${savedTutorSubjects.length} TutorSubject entries for tutor_id ${tutorId}`,
    );
    savedTutorSubjects.forEach((ts, idx) => {
      console.log(
        `  TutorSubject[${idx}]: tutor_subject_id=${ts.tutor_subject_id}, subject_id=${(ts.subject as any)?.subject_id || "N/A"}, subject_name="${subjectNames[idx]}"`,
      );
    });
  ...
  async createBookingRequest(
    ...
    const requestedEnd =
      requestedStart + Math.round(Number(data.duration) * 60);
    ...
    const existing = await this.bookingRequestRepository.find({
      where: {
        tutor: { tutor_id: tutor.tutor_id } as any,
        date: requestedDate,
      },
    ...
    console.log(
      `createBookingRequest: About to save booking with tutor_id=${tutor.tutor_id}, tutor entity:`,
      {
        tutor_id: tutor.tutor_id,
        has_tutor: !!(entity as any).tutor,
        tutor_object: (entity as any).tutor
          ? { tutor_id: ((entity as any).tutor as any).tutor_id }
          : "missing",
      },
    );
    ...
    console.log(
      `createBookingRequest: saved booking id=${(saved as any).id} tutor_id=${tutor.tutor_id} tutor_user_id=${(tutor.user as any)?.user_id} student_user_id=${(student as any)?.user_id}`,
    );
    console.log(`createBookingRequest: booking details:`, {
      ...
      verified_tutor_id:
        (verifyBooking as any)?.tutor?.tutor_id ||
        (verifyBooking as any)?.tutor_id ||
        "NOT FOUND",
    });
    ...
    try {
      console.log(
        `createBookingRequest: Saving notification for tutor user_id=${(tutor.user as any)?.user_id}`,
      );
      const savedNotif = await this.notificationRepository.save(
        tutorNotification as any,
      );
      console.log(
        `createBookingRequest: Notification saved id=${(savedNotif as any)?.notification_id}`,
      );
    } catch (err) {
      console.error(
        "createBookingRequest: Failed to save tutor notification",
        err,
      );
    ...
    if (tutorEmail) {
      ...
      try {
        ...
        console.log(
          `createBookingRequest: New booking request email sent to tutor ${tutorEmail}`,
        );
      } catch (emailErr) {
        console.error(
          "createBookingRequest: Failed to send new booking request email to tutor",
          emailErr,
        );
      }
    }
  ...
  async getStudentBookingRequests(studentUserId: number) {
    ...
    try {
      console.log(
        `getStudentBookingRequests: returning ${requests.length} bookings for student_user_id=${studentUserId}`,
      );
      requests.forEach((r) =>
        console.log(
          `  booking id=${(r as any).id} status=${(r as any).status} tutee_rating=${(r as any).tutee_rating}`,
        ),
      );
    } catch (e) {
  ...
  async getTutorStatus(idParam: number) {
    ...
    if (rawQuery && rawQuery.length > 0) {
      ...
      console.log(
        `[getTutorStatus] 🔍 Raw query found nothing, trying repository query...`,
      );
      tutor = await this.tutorsRepository.findOne({
      ...
      if (!tutor) {
        ...
        console.log(
          `[getTutorStatus] 🔍 Not found by user_id, trying tutor_id: ${idParam}`,
        );
        tutor = await this.tutorsRepository.findOne({
    ...
    if (!tutor) {
      console.error(
        `[getTutorStatus] ❌ Tutor not found for either user_id or tutor_id: ${idParam}`,
      );
  ...
  async updateOnlineStatus(
    ...
    try {
      ...
      await this.tutorsRepository.save(tutor);
      console.log(
        `Tutor ${tutor.tutor_id} (user_id: ${userId}) online status updated to: ${status}`,
      );
    } catch (error) {
      console.error(
        `Failed to update online status for user_id ${userId}:`,
        error,
      );
      throw error;
    }
  ...
  async getTutorProfile(userId: number) {
    ...
    const approvedSubjects =
      tutor.subjects?.filter((ts) => ts.status === "approved") || [];
  ...
  async updateTutorProfile(
    userId: number,
    data: { bio?: string; gcash_number?: string },
  ) {
    const tutor = await this.tutorsRepository.findOne({
      where: { user: { user_id: userId } },
      relations: ["user"],
  ...
  async getTutorSubjectApplications(tutorId: number) {
    ...
    console.log(
      `Found ${applications.length} subject applications for tutor ${tutorId}`,
    );
    applications.forEach((app) => {
      console.log(
        `Application ${app.tutor_subject_id}: ${app.subject.subject_name} - ${app.status} - Notes: ${app.admin_notes || "None"}`,
      );
    });
  ...
  async updateTutorSubjectStatus(
    tutorSubjectId: number,
    status: "approved" | "rejected",
    adminNotes?: string,
  ) {
    console.log("Updating tutor subject status:", {
      tutorSubjectId,
      status,
      adminNotes,
    });
  ...
  async submitSubjectApplication(
    ...
    subjectName: string,
    files: any[],
    isReapplication: boolean = false,
  ) {
    try {
      console.log("Starting subject application submission:", {
        tutorId,
        subjectName,
      ...
      if (courseEntity) {
        ...
        if (!subject) {
          ...
          if (existingWithDifferentCourse) {
            ...
            if (
              ...
              console.log(
                `Subject "${trimmedName}" exists in course_id ${existingCourseId}, creating new subject for course_id ${courseEntity.course_id}`,
              );
              // Create new subject for this course - same name but different course_id
              const created = this.subjectRepository.create({
                subject_name: trimmedName,
              ...
              console.log(
                `Created new subject "${trimmedName}" with course_id ${subject?.course?.course_id || courseEntity.course_id}, subject_id: ${subject.subject_id} (duplicate name allowed for different course)`,
              );
            } else if (!existingCourseId) {
              // If it exists without a course, attach it to this course
              existingWithDifferentCourse.course = courseEntity;
              ...
              console.log(
                `Updated existing subject "${trimmedName}" to have course_id ${subject?.course?.course_id || courseEntity.course_id}, subject_id: ${subject.subject_id}`,
              );
            }
          } else {
            // No existing subject found, create new subject for this course
            ...
            console.log(
              `Created new subject "${trimmedName}" with course_id ${subject?.course?.course_id || courseEntity.course_id}, subject_id: ${subject.subject_id}`,
            );
          }
        } else {
        ...
        if (!subject) {
          ...
          subject = await this.subjectRepository.save(created);
          console.log(
            `Created new subject "${trimmedName}" without course, subject_id: ${subject.subject_id}`,
          );
        } else {
          // Reload with relations for consistency
          subject =
          ...
          console.log(
            `Found existing subject "${trimmedName}" without course, subject_id: ${subject.subject_id}`,
          );
        }
      }
      ...
      if (tutorCourseId && subject) {
        ...
        if (subjectCourseId && subjectCourseId !== tutorCourseId) {
          throw new Error(
            `Subject "${trimmedName}" belongs to course ID ${subjectCourseId}, but tutor is registered with course ID ${tutorCourseId}. Cannot associate subject from different course.`,
          );
        }
      }
      ...
      if (existingTutorSubject) {
        if (existingTutorSubject.status === "approved") {
          throw new Error(
            "You have already been approved for this subject expertise",
          );
          ...
          if (files && files.length > 0) {
            console.log(
              "Found existing pending TutorSubject, attaching documents to it",
            );
            ...
            throw new Error(
              "You have already applied for this subject expertise and it is pending review",
            );
          }
          ...
          savedTutorSubject =
            await this.tutorSubjectRepository.save(existingTutorSubject);
          ...
          if (isReapplication && (!files || files.length === 0)) {
            ...
            if (existingDocs.length > 0) {
              console.log(
                `Reapplying rejected subject "${trimmedName}" with ${existingDocs.length} existing documents (no new files)`,
              );
              // Status already changed to pending above, existing documents remain
              return {
                success: true,
                message: "Subject reapplication submitted successfully",
              };
            }
          }
        }
        ...
        if (!files || files.length === 0) {
          throw new Error(
            "At least one file is required for new subject application",
          );
        }
      ...
      if (files && files.length > 0) {
        try {
          console.log(
            "Creating documents for tutor subject:",
            savedTutorSubject.tutor_subject_id,
          );
          console.log(
            "Subject name:",
            trimmedName,
          ...
          const existingDocs = await this.tutorSubjectDocumentRepository.find({
            where: {
              tutorSubject: {
                tutor_subject_id: savedTutorSubject.tutor_subject_id,
              } as any,
            },
          });
          if (existingDocs.length > 0) {
            console.log(
              `Clearing ${existingDocs.length} existing documents for tutor subject ${savedTutorSubject.tutor_subject_id}`,
            );
          ...
          console.log(
            `Successfully saved ${savedDocuments.length} document(s) for tutor subject:`,
            savedTutorSubject.tutor_subject_id,
          );
          ...
          if (!savedDocuments || savedDocuments.length === 0) {
            throw new Error(
              "Failed to save documents - no documents were saved",
            );
          }
        } catch (error) {
          ...
          throw new Error(
            `Failed to save documents for subject "${trimmedName}": ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        }
      } else {
        console.log(
          "No files provided for tutor subject:",
          savedTutorSubject.tutor_subject_id,
        );
      }
      ...
      return {
        success: true,
        tutorSubjectId: savedTutorSubject.tutor_subject_id,
      };
    } catch (error) {
  ...
  async getBookingRequests(userId: number) {
    ...
    if (!tutor) {
      console.warn(
        `getBookingRequests: Tutor not found for id/user_id=${userId}`,
      );
      throw new NotFoundException("Tutor not found");
    }

    console.log(
      `getBookingRequests: Looking for bookings with tutor_id=${tutor.tutor_id}`,
    );
    ...
    const rawCount = await this.bookingRequestRepository
      ...
      .getCount();
    console.log(
      `getBookingRequests: Raw count of bookings with tutor_id=${tutor.tutor_id}: ${rawCount}`,
    );
    ...
    console.log(
      `getBookingRequests: found ${requests.length} requests for tutor_id=${tutor.tutor_id}`,
    );
    console.log(
      `getBookingRequests: booking IDs:`,
      requests.map((r: any) => ({
        id: r.id,
        subject: r.subject,
        status: r.status,
        date: r.date,
        tutor_id_in_booking:
          (r.tutor as any)?.tutor_id || (r as any).tutor_id || "missing",
    ...
    console.log(
      `getBookingRequests: Alternative query found ${altRequests.length} requests`,
    );
  ...
  async updateBookingRequestStatus(
    ...
    const savedNotification =
      await this.notificationRepository.save(studentNotification);
    console.log(
      `updateBookingRequestStatus: Created notification for student user_id=${student.user_id} about booking approval`,
    );
    console.log(
      `updateBookingRequestStatus: Notification ID=${savedNotification.notification_id}, userId=${savedNotification.userId}, userType=${savedNotification.userType}`,
    );
    console.log(
      `updateBookingRequestStatus: Message=${notificationMessage.substring(0, 100)}...`,
    );
    ...
    if (status === "accepted") {
      try {
        ...
        console.log(
          `updateBookingRequestStatus: Booking approval email sent to ${(student as any).email}`,
        );
      } catch (emailErr) {
        console.error(
          "updateBookingRequestStatus: Failed to send booking approval email",
          emailErr,
        );
      }
    }
    ...
    if (status === "declined") {
      try {
        ...
        console.log(
          `updateBookingRequestStatus: Booking declined email sent to ${(student as any).email}`,
        );
      } catch (emailErr) {
        console.error(
          "updateBookingRequestStatus: Failed to send booking declined email",
          emailErr,
        );
      }
    }
    ...
    if (tutorUserEmail) {
      try {
        ...
        console.log(
          `updateBookingRequestStatus: Booking action confirmation email sent to tutor ${tutorUserEmail}`,
        );
      } catch (emailErr) {
        console.error(
          "updateBookingRequestStatus: Failed to send booking action confirmation email to tutor",
          emailErr,
        );
      }
    }
    ...
    if (status === "accepted") {
      ...
      console.log(
        `updateBookingRequestStatus: Created notification for tutor user_id=${(tutor.user as any)?.user_id} about approval`,
      );
    }
  ...
  async updatePaymentStatus(
    ...
    if (status !== "approved") {
      ...
      await Promise.all(
        notifications.map((n) => this.notificationRepository.save(n)),
      );
    }
  ...
  async markBookingAsCompleted(
    ...
    try {
      ...
      const sessionDate = new Date(request.date).toLocaleDateString("en-PH", {
        year: "numeric",
        month: "long",
        day: "numeric",
      });
  ...
  async uploadPaymentProof(bookingId: number, file: any) {
    ...
    if (
      ...
      throw new BadRequestException(
        "Cannot upload payment proof for this booking status",
      );
    }
  ...
  async getTutorPayments(userId: number) {
    ...
    if (!tutor) {
      console.warn(
        `getTutorPayments: Tutor not found for id/user_id=${userId}`,
      );
      throw new NotFoundException("Tutor not found");
    }
  ...
  async getTutorEarningsStats(userId: number) {
    ...
    if (!tutor) {
      console.warn(
        `getTutorEarningsStats: Tutor not found for id/user_id=${userId}`,
      );
      throw new NotFoundException("Tutor not found");
    }
    ...
    const confirmedPayments = payments.filter(
      (p: any) => p.status === "confirmed" || p.status === "paid",
    );
    const pendingPayments = payments.filter((p: any) => p.status === "pending");

    const total_earnings = confirmedPayments.reduce(
      (sum: number, p: any) => sum + Number(p.amount),
      0,
    );
    const pending_earnings = pendingPayments.reduce(
      (sum: number, p: any) => sum + Number(p.amount),
      0,
    );
    ...
    const total_hours = completedBookings.reduce(
      (sum: number, b: any) => sum + Number(b.duration || 0),
      0,
    );
  ...
  private async deleteOldProfileImages(userId: number) {
    ...
    const userProfileImagesPath = path.join(
      process.cwd(),
      "user_profile_images",
    );
    ...
    try {
      ...
      const filesToDelete = files.filter((file) => {
        ...
        const isCurrentFile =
          ...
          file === path.basename(user.profile_image_url);
        return matchesPattern && !isCurrentFile; // Don't delete the current file
      });

      console.log(
        `Found ${filesToDelete.length} old profile image files to delete for user ${userId}:`,
        filesToDelete,
      );
  ...
  private async deleteOldGcashQRFiles(userId: number) {
    ...
    try {
      ...
      const filesToDelete = files.filter((file) => {
        ...
        const isCurrentFile =
          tutor.gcash_qr_url && file === path.basename(tutor.gcash_qr_url);
        return matchesPattern && !isCurrentFile; // Don't delete the current file
      });

      console.log(
        `Found ${filesToDelete.length} old GCash QR files to delete for user ${userId}:`,
        filesToDelete,
      );
