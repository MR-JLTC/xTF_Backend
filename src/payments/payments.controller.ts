...
export class PaymentsController {
  ...
  @Patch(":id/dispute")
  updateDispute(@Param("id") id: string, @Body() dto: UpdatePaymentDisputeDto) {
  ...
  async verifyPayment(
    @Param("id") id: string,
    @Body()
    body: { status: "confirmed" | "rejected"; rejection_reason?: string },
    ...
    try {
      ...
      if (adminProof && adminProof.buffer) {
        ...
        console.log(
          `PaymentsController.verifyPayment: Uploading adminProof to Supabase as ${filename}`,
        );
        ...
        console.log(
          "PaymentsController.verifyPayment: No adminProof file received in request",
        );
      }
      if (body.status === "rejected" && body.rejection_reason) {
        console.log(
          `PaymentsController.verifyPayment: Rejection reason: ${body.rejection_reason}`,
        );
      }
      const res = await this.paymentsService.verifyPayment(
        +id,
        body.status,
        fileForService,
      ...
      console.error(
        "PaymentsController.verifyPayment: Error while verifying payment:",
        err,
      );
      throw err;
    }
  ...
  @Patch(":id/confirm")
  async confirmByTutor(@Param("id") id: string, @Request() req: any) {
