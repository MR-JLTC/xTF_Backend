...
export class CoursesService {
  ...
  async createCourse(
    ...
    if (existingCourse) {
      throw new BadRequestException(
        `Course "${courseName}" already exists for this university.`,
      );
    }
    ...
    if (courseWithMatchingAcronym) {
      throw new BadRequestException(
        `Course name "${courseName}" conflicts with an existing course acronym "${courseWithMatchingAcronym.acronym}" for this university.`,
      );
    }
  ...
  async updateCourse(
    courseId: number,
    body: { course_name?: string; university_id?: number; acronym?: string },
  ): Promise<Course> {
    ...
    const newUniversityId =
      body.university_id || course.university.university_id;
    ...
    if (body.course_name || body.university_id) {
      ...
      if (existingCourse && existingCourse.course_id !== courseId) {
        throw new BadRequestException(
          `Course "${newCourseName}" already exists for this university.`,
        );
      }
    }
    ...
    if (body.course_name) {
      const courseWithMatchingAcronym = await this.coursesRepository
        .createQueryBuilder("course")
        .where("LOWER(course.acronym) = LOWER(:courseName)", {
          courseName: newCourseName,
        })
        .andWhere("course.university_id = :universityId", {
      ...
      if (courseWithMatchingAcronym) {
        throw new BadRequestException(
          `Course name "${newCourseName}" conflicts with an existing course acronym "${courseWithMatchingAcronym.acronym}" for this university.`,
        );
      }
    }
