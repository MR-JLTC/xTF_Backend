...
export class DashboardService {
  ...
  async getStats() {
    ...
    const paymentStatusCounts = paymentStatusCountsRaw.reduce(
      (acc, row) => {
    ...
    const recentConfirmedRevenue = Number(
      (parseFloat(recentPaymentsSumRaw?.sum || "0") || 0).toFixed(2),
    );
    ...
    if (uniIds.length) {
      ...
      universities.forEach((u) => {
        (uniMap as any)[(u as any).university_id] = (u as any).name;
      });
    }
    const uniAgg: Record<
      number,
      { university: string; tutors: number; tutees: number }
    > = {};
    tutorUniRaw.forEach((r: any) => {
      const id = Number(r.university_id);
      if (!id) return;
      uniAgg[id] = uniAgg[id] || {
        university: uniMap[id] || "Unknown",
        tutors: 0,
        tutees: 0,
      };
    ...
    const universityDistribution = Object.values(uniAgg).sort(
      (a, b) => b.tutors + b.tutees - (a.tutors + a.tutees),
    );

    // Overall users by type (approved tutors only)
    const tutorsTotal = await this.tutorsRepository.count({
      where: { status: "approved" as any },
    });
    // Prefer counting active tutee users; fallback to students table if needed
    let tuteesTotal = await this.usersRepository.count({
      where: { user_type: "tutee" as any },
    });
    if (!tuteesTotal) {
    ...
    if (courseIds.length) {
      ...
      courses.forEach((c) => {
        (courseMap as any)[(c as any).course_id] =
          (c as any).course_name || (c as any).name;
      });
    }
    const courseAgg: Record<
      number,
      { courseName: string; tutors: number; tutees: number }
    > = {};
    tutorCourseRaw.forEach((r: any) => {
      const id = Number(r.course_id);
      if (!id) return;
      courseAgg[id] = courseAgg[id] || {
    ...
    const courseDistribution = Object.values(courseAgg).sort(
      (a, b) => b.tutors + b.tutees - (a.tutors + a.tutees),
    );
    ...
    const subjectSessionsRaw = await this.bookingRequestRepository
      ...
      .where("booking.status IN (:...statuses)", {
        statuses: ["completed", "admin_payment_pending"],
      })
      .groupBy("booking.subject")
      .orderBy("sessions", "DESC")
      .getRawMany();

    const subjectSessions = subjectSessionsRaw
      .map((r: any) => {
